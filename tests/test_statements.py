...
def test_get_all_transactions(client, db, auth_headers):
    ...
    with client.application.app_context():
        ...
        transactions = [
            Transaction(pdf_id=pdf.id, date="2024-01-15",
                        description="Mercado", amount=-100.0, category="Alimentação"),
            Transaction(pdf_id=pdf.id, date="2024-01-20",
                        description="Posto", amount=-80.0, category="Transporte"),
            Transaction(pdf_id=pdf.id, date="2024-02-05",
                        description="Shopping", amount=-200.0, category="Vestuário"),
        ]
    ...
    response = client.get(
        '/transactions?start_date=2024-02-01', headers=auth_headers)
    assert response.status_code == 200
    data = response.json
    assert len(data) == 1
    ...
    response = client.get(
        '/transactions?category=Alimentação', headers=auth_headers)
    assert response.status_code == 200
    data = response.json
    assert len(data) == 1
...
def test_get_transaction_summary(client, db, auth_headers):
    ...
    with client.application.app_context():
        ...
        transactions = [
            Transaction(pdf_id=pdf.id, date="2024-01-15",
                        description="Compra cartão", amount=-100.0),
            Transaction(pdf_id=pdf.id, date="2024-01-20",
                        description="Pix recebido", amount=500.0),
            Transaction(pdf_id=pdf.id, date="2024-02-05",
                        description="Boleto pago", amount=-150.0),
        ]
        db.session.add_all(transactions)
        db.session.commit()
...
def test_get_spending_by_category(client, db, auth_headers):
    ...
    with client.application.app_context():
        ...
        transactions = [
            Transaction(pdf_id=pdf.id, date="2024-01-15",
                        description="Supermercado", amount=-200.0, category="Alimentação"),
            Transaction(pdf_id=pdf.id, date="2024-01-20",
                        description="Posto", amount=-150.0, category="Transporte"),
            Transaction(pdf_id=pdf.id, date="2024-01-25",
                        description="Mercado", amount=-100.0, category="Alimentação"),
        ]
        db.session.add_all(transactions)
        db.session.commit()
...
# ⚠️ SQLite não suporta o tipo LOB do Postgres. Este teste foi comentado por agora.
...
def test_delete_pdf(client, db, auth_headers):
    ...
    with client.application.app_context():
        ...
        transaction = Transaction(
            pdf_id=pdf.id, date="2024-01-15", description="Test", amount=-50.0)
        db.session.add(transaction)
        db.session.commit()
