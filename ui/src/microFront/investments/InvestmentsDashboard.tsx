...
}> = ({ totalInvested, expectedValue, percentageChange }) => {
      ...
      <div className="inv-card">
        <div className="inv-card-head">
          <div>
            ...
            <p className="inv-card-value">
              R${" "}
              {expectedValue.toLocaleString("pt-BR", {
                minimumFractionDigits: 2,
              })}
            </p>
...
const CryptoList: React.FC<{ data: CryptoRow[] }> = ({ data }) => (
  <div className="inv-panel">
    ...
    <div className="inv-list">
      {data.map((c, i) => {
        const pnlPct =
          ((c.price * (c.invested / Math.max(c.price, 1)) - c.invested) /
            Math.max(c.invested, 1)) *
          100;
        return (
          <div className="inv-list-row" key={i}>
            <div>
              ...
              <p className="inv-list-subtle">
                Investido: ${c.invested.toFixed(2)}
              </p>
            </div>
            <div className="inv-list-right">
              <p className="inv-list-price">${c.price.toFixed(2)}</p>
              <p className={`inv-list-change ${c.change >= 0 ? "up" : "down"}`}>
                {c.change >= 0 ? "+" : ""}
                {c.change.toFixed(2)}% 24h
              </p>
              <p className={`inv-list-change ${pnlPct >= 0 ? "up" : "down"}`}>
                P/L: {pnlPct >= 0 ? "+" : ""}
                {pnlPct.toFixed(2)}%
              </p>
            </div>
        ...
        <div className="inv-list-row" key={i}>
          <div>
            ...
            <p className="inv-list-subtitle">
              {b.type} • Venc: {b.maturity}
            </p>
            <p className="inv-list-subtle">
              Investido: R$ {b.invested.toFixed(2)}
            </p>
          </div>
          <div className="inv-list-right">
            <p className="inv-list-price">{b.rate.toFixed(2)}% a.a.</p>
            <p className="inv-list-subtle">
              Rendido: R$ {b.accrued.toFixed(2)}
            </p>
          </div>
        </div>
      ))}
  ...
  <div className="inv-panel">
    ...
    <div className="inv-list">
      {data.map((s, i) => {
        ...
        const pnlPct =
          ((s.price * units - s.invested) / Math.max(s.invested, 1)) * 100;
        return (
          <div className="inv-list-row" key={i}>
            <div>
              ...
              <p className="inv-list-subtle">
                Investido: R$ {s.invested.toFixed(2)}
              </p>
            </div>
            <div className="inv-list-right">
              <p className="inv-list-price">R$ {s.price.toFixed(2)}</p>
              <p className={`inv-list-change ${s.change >= 0 ? "up" : "down"}`}>
                {s.change >= 0 ? "+" : ""}
                {s.change.toFixed(2)}% dia
              </p>
              <p className={`inv-list-change ${pnlPct >= 0 ? "up" : "down"}`}>
                P/L: {pnlPct >= 0 ? "+" : ""}
                {pnlPct.toFixed(2)}%
...
const InvestmentsDashboard: React.FC = () => {
  ...
  const expectedValue = useMemo(() => {
    const c = cryptoData.reduce(
      (sum, x) =>
        sum + x.price * Math.max(x.invested / Math.max(x.price, 1), 1),
      0
    );
    const s = stocksData.reduce(
      (sum, x) =>
        sum + x.price * Math.max(x.invested / Math.max(x.price, 1), 1),
      0
    );
    const f = fixedIncomeData.reduce(
      (sum, x) => sum + x.invested + x.accrued,
      0
    );
    return c + s + f;
  }, [cryptoData, stocksData, fixedIncomeData]);
  ...
  const percentageChange =
    totalInvested > 0
      ? ((expectedValue - totalInvested) / totalInvested) * 100
      : 0;
  ...
  const fetchHistoricalInRange = async (
    id: string,
    start: Date,
    end: Date,
    signal?: AbortSignal
